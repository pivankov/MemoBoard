/**
 * Данные для посева закладок
 */

export const DUMMY_BOOKMARK_CATEGORIES = [
  { uid: "wxiqnC", parent_uid: "0000", title: "Корзина", icon: "Trash", position: 1 },
  { uid: "ktGDhX", parent_uid: "0000", title: "Несортированные", icon: "Inbox", position: 0 }, 
  { uid: "0000", parent_uid: "", title: "Системные", icon: "", position: 0 },
  { uid: "wTOx", parent_uid: "", title: "Работа", icon: "", position: 1 },
  { uid: "tVhm", parent_uid: "wTOx", title: "Инструменты", icon: "Folder", position: 0 },
  { uid: "IpYb", parent_uid: "", title: "Основное", icon: "", position: 2 },
  { uid: "4x3o", parent_uid: "IpYb", title: "Рецепты", icon: "Folder", position: 1 },
  { uid: "7i95", parent_uid: "IpYb", title: "Игры", icon: "Rocket", position: 2 },
  { uid: "mKgw", parent_uid: "IpYb", title: "Хочу прочитать", icon: "Folder", position: 3 },
  { uid: "V8dU", parent_uid: "IpYb", title: "Хочу посмотреть", icon: "Film", position: 4 },
  { uid: "Arkq", parent_uid: "wTOx", title: "Документации", icon: "Folder", position: 99 },
  { uid: "7LSI", parent_uid: "wTOx", title: "Хочу изучить", icon: "Folder", position: 99 },
  { uid: "HWt3", parent_uid: "wTOx", title: "Letoile", icon: "UserGroup", position: 99 },
  { uid: "l15e", parent_uid: "IpYb", title: "Инструменты", icon: "Folder", position: 0 },
  { uid: "ghhl", parent_uid: "IpYb", title: "MyApp", icon: "Folder", position: 99 },
  { uid: "F6O2", parent_uid: "IpYb", title: "Хочу пересмотреть", icon: "Film", position: 99 },
  { uid: "Vzgd", parent_uid: "IpYb", title: "Хочу поиграть", icon: "Folder", position: 99 },
  { uid: "gKdY", parent_uid: "IpYb", title: "Sur Ron", icon: "Folder", position: 99 },
  { uid: "j5cG", parent_uid: "IpYb", title: "Велосипед", icon: "Folder", position: 99 },
  { uid: "ie4p", parent_uid: "IpYb", title: "LG", icon: "Folder", position: 99 },
  { uid: "wai5", parent_uid: "IpYb", title: "OnePlus 13R", icon: "Folder", position: 99 },
  { uid: "SVSZ", parent_uid: "IpYb", title: "Кофе", icon: "Folder", position: 99 },
  { uid: "euMD", parent_uid: "IpYb", title: "AI", icon: "Folder", position: 99 },
  { uid: "eH1u", parent_uid: "wTOx", title: "Поиск 2025", icon: "Folder", position: 99 },
];

export const DUMMY_BOOKMARK_TAGS = [
  { uid: "GE20b", title: "Срочно" },
  { uid: "8GN6p", title: "Xbox" },
  { uid: "fM6YW", title: "Курица" },
  { uid: "x78IZ", title: "Ребрышки" },
  { uid: "NSMjl", title: "Playstation" },
  { uid: "ent7g", title: "Node" },
  { uid: "xU4PD", title: "SQLite" },
  { uid: "1fFZi", title: "Javascript" },
  { uid: "Wsw8g", title: "JSON" },
  { uid: "ulV4b", title: "Свинина" },
  { uid: "S7XFX", title: "Картофель" },
  { uid: "7Mjrx", title: "nvm" },
  { uid: "gY5YF", title: "SQL" },
  { uid: "Pe8tG", title: "Git" },
  { uid: "CyVhI", title: "Хочу приготовить" },
  { uid: "ZJf5O", title: "Хочу изучить" },
  { uid: "ljFAI", title: "Где кататься" },
  { uid: "ymCVP", title: "Кофе" },
  { uid: "XtI6V", title: "Android" },
];

export const DUMMY_BOOKMARKS = [
  {
    uid: "dSMicJ",
    category_uid: "ktGDhX",
    title: "Полный гайд на Резюме в IT - Как Правильно составить резюме программисту?",
    url: "https://www.youtube.com/watch?v=mU-MghntMxg",
    description: "Enjoy the videos and music that you love, upload original content and share it all with friends, family and the world on YouTube.",
    tags: ["ZJf5O"],
    preview: "",
    created_at: "2025-06-30 02:38:20",
    updated_at: "2025-06-30 02:38:27",
    transition_counter: 1,
    favorite: false,
  },
  {
    uid: "UIje1I",
    category_uid: "ktGDhX",
    title: " Nest + Typia = Nestia. Доминация Typescript",
    url: "https://www.youtube.com/watch?v=eAFq35Z6z5I",
    description: "Enjoy the videos and music that you love, upload original content and share it all with friends, family and the world on YouTube.",
    tags: ["ZJf5O", "1fFZi"],
    preview: "",
    created_at: "2025-06-30 02:37:45",
    updated_at: "2025-06-30 02:38:00",
    transition_counter: 1,
    favorite: false,
  },
  {
    uid: "qRN4eu",
    category_uid: "ktGDhX",
    title: "\"Star Rail LIVE 2025\" Concert Full Recording | Honkai: Star Rail - YouTube",
    url: "https://www.youtube.com/watch?v=FAckVYINs8E",
    description: "The Official Recording of \"Star Rail LIVE\" Honkai: Star Rail Concert 2025 is Online — The Cosmos Resonates for You!Attention, passengers, the Express is abou...",
    tags: [],
    preview: "https://i.ytimg.com/vi/FAckVYINs8E/maxresdefault.jpg",
    created_at: "2025-05-04 21:51:58",
    updated_at: "2025-05-04 21:52:00",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "NUjbfo",
    category_uid: "ktGDhX",
    title: "Lenovo Legion Y700 Как настроить планшет , установить русский язык , удалить китайские приложения - YouTube",
    url: "https://www.youtube.com/watch?v=EsyqqjnwOws",
    description: "Lenovo Legion Y700 Китайская версия с китайской оригинальной прошивкой с русским языком .Настрока планшета,установка русского языка,удаление китайских прилож...",
    tags: [],
    preview: "https://i.ytimg.com/vi/EsyqqjnwOws/maxresdefault.jpg?sqp=-oaymwEmCIAKENAF8quKqQMa8AEB-AH-CYAC0AWKAgwIABABGFcgWChlMA8=&rs=AOn4CLBUFjk2kmMryRmpuJJJ90k721l0HQ",
    created_at: "2025-03-14 13:51:00",
    updated_at: "2025-03-14 13:51:03",
    transition_counter: 2,
    favorite: false,
  },
  {
    uid: "MmD8cr",
    category_uid: "ktGDhX",
    title: "Встреча христианского клуба НИУ ВШЭ | иерей Павел Островский - YouTube",
    url: "https://www.youtube.com/watch?v=ZN7YTvMaw0w",
    description: "Как найти ответы на сложные вопросы о вере? Где обсудить важные темы на простом, доступном языке? На встрече с иереем Павлом Островским в Культурном центре «...",
    tags: [],
    preview: "https://i.ytimg.com/vi/ZN7YTvMaw0w/maxresdefault.jpg",
    created_at: "2025-03-08 22:28:58",
    updated_at: "2025-03-08 22:29:00",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "pIDIZR",
    category_uid: "ktGDhX",
    title: "Дмитрий Лаврик - YouTube",
    url: "https://www.youtube.com/@dmitrylavrik/videos",
    description: "Полезные материалы по программированию и веб-разработке",
    tags: ["ZJf5O", "1fFZi"],
    preview: "https://yt3.googleusercontent.com/ytc/AIdro_kSO4ok-Ezl1xicKfrqMmRiKCuw_fg8cvpIO2LAXggJox4=s900-c-k-c0x00ffffff-no-rj",
    created_at: "2025-03-07 19:02:06",
    updated_at: "2025-03-07 19:02:10",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "W9QGHV",
    category_uid: "ktGDhX",
    title: "VK Лекторий | Москва, VK Fest | 2024 | Павел Островский: Лайки, юмор и семья",
    url: "https://vkvideo.ru/video-165631795_456239998",
    description: "",
    tags: [],
    preview: "",
    created_at: "2025-03-04 22:33:14",
    updated_at: "2025-03-05 20:46:35",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "qczCGW",
    category_uid: "ktGDhX",
    title: "СОВСЕМ НЕВЫДУМАННАЯ ИСТОРИЯ (РХ 2024)",
    url: "https://vk.com/video-38080110_456239734",
    description: "",
    tags: [],
    preview: "",
    created_at: "2025-03-02 23:17:07",
    updated_at: "2025-03-02 23:17:16",
    transition_counter: 2,
    favorite: false,
  },
  {
    uid: "tqGXiv",
    category_uid: "ktGDhX",
    title: "Введение в Node.js. Часть 1. Знакомство с языком - YouTube",
    url: "https://www.youtube.com/watch?v=kkU_Lk8kdms",
    description: "- общие концепция Node- однопоточный или нет, идея event loop- ключевые отличия от Node от PHP- hello world server- почему сейчас я всё чаще выбираю node для...",
    tags: ["ZJf5O", "ent7g", "1fFZi"],
    preview: "https://i.ytimg.com/vi/kkU_Lk8kdms/maxresdefault.jpg",
    created_at: "2025-01-14 18:05:59",
    updated_at: "2025-01-14 18:06:01",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "5kvx9r",
    category_uid: "ktGDhX",
    title: "Ошибки приготовления кофе в воронке | Hario v60",
    url: "https://www.youtube.com/watch?v=VJ07UreDgII",
    description: "В этом видео наш оператор и арт-директор Федор впервые готовит кофе в воронке! \n\nИспользуя классику альтернативы — Hario v60, он попробует приготовить напиток по своему рецепту, а если что-то пойдёт не так, Николай внесёт свои корректировки. Или не внесёт?",
    tags: ["ZJf5O", "ymCVP"],
    preview: "",
    created_at: "2024-12-15 16:50:21",
    updated_at: "2024-12-15 16:50:49",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "arl0na",
    category_uid: "ktGDhX",
    title: "КОНЕЦ СВЕТА БЛИЗКО?! Мысли математика и священника. Савватеев и Островский - YouTube",
    url: "https://www.youtube.com/watch?v=hlfjUaCugXw",
    description: "▶︎ Подарите ненужным гаджетам новую жизнь вместе с Авито - https://bit.ly/3VfSvr4?erid=LjN8KL8SV▶︎ Компактный и удобный компрессорный ингалятор Армед для ком...",
    tags: [],
    preview: "https://i.ytimg.com/vi/hlfjUaCugXw/maxresdefault.jpg",
    created_at: "2024-12-02 15:35:20",
    updated_at: null,
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "hiqXlv",
    category_uid: "ktGDhX",
    title: "BMW k1600gtl полезный и бесполезный тюнинг - YouTube",
    url: "https://www.youtube.com/watch?v=U2oHuW47O8I",
    description: "Всем привет, в сегодняшнем видео я решил рассказать про доработки, которые планирую произвести к сезону со своим мотоциклом BMW k1600gtl. Также будет актуаль...",
    tags: [],
    preview: "https://i.ytimg.com/vi/U2oHuW47O8I/maxresdefault.jpg",
    created_at: "2024-12-02 15:21:44",
    updated_at: null,
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "WEhe8e",
    category_uid: "ktGDhX",
    title: "Будущие работы: какие навыки будут востребованы - YouTube",
    url: "https://www.youtube.com/watch?v=3N-cOhvR5Ro",
    description: "Программа для тех, кто хочет стать востребованным программистом: https://bit.ly/3w46rv5Закрытое сообщество единомышленников: https://t.me/NextgenSocialBotМой...",
    tags: [],
    preview: "https://i.ytimg.com/vi/3N-cOhvR5Ro/maxresdefault.jpg",
    created_at: "2024-12-02 15:21:34",
    updated_at: null,
    transition_counter: 1,
    favorite: false,
  },
  {
    uid: "vdzTF8",
    category_uid: "ktGDhX",
    title: "По ту сторону изгороди (мультсериал, 2014) – смотреть онлайн все 10 видео от По ту сторону изгороди (мультсериал, 2014) в хорошем качестве на RUTUBE",
    url: "https://rutube.ru/plst/328979/",
    description: "Смотреть видеоподборку По ту сторону изгороди (мультсериал, 2014) канала  в хорошем качестве без регистрации и совершенно бесплатно на RUTUBE пользователя По ту сторону изгороди (мультсериал, 2014) (328979).",
    tags: [],
    preview: "https://pic.rutubelist.ru/video/1b/42/1b42df069c5e871abfa2ccc7e9e4e02f.jpg",
    created_at: "2024-11-05 12:24:03",
    updated_at: "2024-11-05 12:24:07",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "GD7vMj",
    category_uid: "ktGDhX",
    title: "РОСЛА В МОНАСТЫРЕ СЕРГИЯ РОМАНОВА И НЕ МОГЛА УЙТИ.ПОБОИ, МАНИПУЛЯЦИИ И ЛОЖЬ. ПОМОЛЧИМ В КАВЫЧКАХ #22 - YouTube",
    url: "https://www.youtube.com/watch?v=h4s2eGe7TKY",
    description: "Алиса Крыжановская, сегодня популярный блогер, а когда-то – простой подросток, живший в стенах Среднеуральского женского монастыря. Теперь многие забыли о то...",
    tags: [],
    preview: "https://i.ytimg.com/vi/h4s2eGe7TKY/maxresdefault.jpg",
    created_at: "2024-10-24 12:09:31",
    updated_at: "2024-10-24 12:09:33",
    transition_counter: null,
    favorite: false,
  },
  {
    uid: "vhZ68X",
    category_uid: "ktGDhX",
    title: "Амиран.ИИ Спасение Или Трагедия Для Человека_ Ольга Ускова. | ВКонтакте",
    url: "https://vk.com/video-227461322_456239326",
    description: "",
    tags: [],
    preview: "",
    created_at: "2024-10-18 13:04:07",
    updated_at: "2024-10-18 13:04:20",
    transition_counter: 3,
    favorite: false,
  },
  {
    uid: "xA6Imd",
    category_uid: "ktGDhX",
    title: "Дебаты:Ричард Докинз и Роуэн Уильямс",
    url: "https://www.youtube.com/watch?v=nGi6sBU-zRc",
    description: "",
    tags: [],
    preview: "",
    created_at: "2024-10-16 11:44:57",
    updated_at: "2024-10-16 11:45:15",
    transition_counter: null,
    favorite: false,
  },
];
