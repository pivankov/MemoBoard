  ...
  rules: {
    ...
    'simple-import-sort/imports': ['error', {
      groups: [
        ...
        // 4. Внутренние абсолютные импорты из src (baseUrl=src)
        ['^(assets|components|enums|hooks|layouts|pages|types|utils)(/.*)?$'],

        // 5. Прочие внешние зависимости
        ['^@?\\w'],

        // 6. Абсолютные алиасы проекта (если появятся)
        ['^@/'],

        // 7. Относительные импорты: родительские и соседние
        ['^\.\.(?!/?$)', '^\.\./?$', '^\./(?=.*/)(?!/?$)', '^\.(?!/?$)', '^\./?$'],

        // 8. Стили и ассеты в самом конце
